        ...
        if KEYS.check_pressed(pg.K_LCTRL):
            ...
            if KEYS.down[pg.K_v]:
                if isinstance(COPY, GameObject):
                    ...
                    parameters[4] = MOUSE.scene_position
                    new_game_object = create_Object_from_parameters(parameters, SCENE.value)
                    SELECTED_OBJ.value = new_game_object
            if KEYS.check_pressed(pg.K_UP) or KEYS.check_pressed(pg.K_DOWN) or KEYS.check_pressed(pg.K_RIGHT) or KEYS.check_pressed(pg.K_LEFT):
                cam_speed = 50
                CAMERA.pos += fps_factor * cam_speed * vec2(KEYS.check_pressed(pg.K_RIGHT)-KEYS.check_pressed(pg.K_LEFT), KEYS.check_pressed(pg.K_DOWN)-KEYS.check_pressed(pg.K_UP))
        if KEYS.check_down(pg.K_BACKSPACE):
            selected_obj:GameObject = get_global("<Selected_Object>").value
            if selected_obj is not None:
                if not selected_obj.engine_generated:
                    Raise_Error(main_window_manager, "Game Objects generated by code can't be deleted")
                else:
                    delete_game_object(selected_obj)
...
def tick_scene():
    ...
    if ALLOW_SPEED_UP:
        dt = 1/DELTA_FPS
    fps_factor=60*dt
    tick_game_objects(fps_factor, dt, update=False) # no update for engine pre-view
